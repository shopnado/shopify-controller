...
import (
	...
	listers "github.com/shopnado/shopify-controller/controller/generated/listers/controller.shopnado.xyz/v1"
...
func (c *eventController) UpdateStatus(obj *v1.Event) (*v1.Event, error) {
	return c.clientGetter.Events(obj.Namespace).UpdateStatus(context.TODO(), obj, metav1.UpdateOptions{})
}
...
type EventStatusHandler func(obj *v1.Event, status v1.EventStatus) (v1.EventStatus, error)

type EventGeneratingHandler func(obj *v1.Event, status v1.EventStatus) ([]runtime.Object, v1.EventStatus, error)

func RegisterEventStatusHandler(ctx context.Context, controller EventController, condition condition.Cond, name string, handler EventStatusHandler) {
	statusHandler := &eventStatusHandler{
		client:    controller,
		condition: condition,
		handler:   handler,
	}
	controller.AddGenericHandler(ctx, name, FromEventHandlerToHandler(statusHandler.sync))
}

func RegisterEventGeneratingHandler(ctx context.Context, controller EventController, apply apply.Apply,
	condition condition.Cond, name string, handler EventGeneratingHandler, opts *generic.GeneratingHandlerOptions) {
	statusHandler := &eventGeneratingHandler{
		EventGeneratingHandler: handler,
		apply:                  apply,
		name:                   name,
	...
	RegisterEventStatusHandler(ctx, controller, condition, name, statusHandler.Handle)
}

type eventStatusHandler struct {
	client    EventClient
	condition condition.Cond
	handler   EventStatusHandler
}

func (a *eventStatusHandler) sync(key string, obj *v1.Event) (*v1.Event, error) {
	if obj == nil {
		return obj, nil
	}
...
type eventGeneratingHandler struct {
...
func (a *eventGeneratingHandler) Remove(key string, obj *v1.Event) (*v1.Event, error) {
	if obj != nil {
		return obj, nil
	}
...
func (a *eventGeneratingHandler) Handle(obj *v1.Event, status v1.EventStatus) (v1.EventStatus, error) {
	objs, newStatus, err := a.EventGeneratingHandler(obj, status)
	if err != nil {
